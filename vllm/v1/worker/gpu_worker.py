...
class Worker(WorkerBase):
    def __init__(
        ...
        #
        # These singletons are process-local; an init call inside
        # DPEngineCoreProc.__init__ (the Actor process) does NOT carry
        # over into the Ray-spawned Worker process. Without this init,
        # the Worker's per-step DP collective (`_run_ar` ->
        # `ft_or_raw_all_reduce`) takes the `ft is None or state is None`
        # branch and falls back to `dist.all_reduce(group=dp_group)` on
        # the original DP-world-size gloo group, which still includes
        # any dead peers. That raw all_reduce has no timeout and can
        # hang the worker indefinitely when the dead peer's gloo socket
        # state hasn't yet been observed as torn down (DYN-3121
        # post-redistribute cascade root cause).
        #
        # By initializing here, the Worker takes the FT-aware path:
        # `DPFTGlooManager.all_reduce` on a survivors-only sub-group
        # that excludes the dead rank, so the call cannot hang on it.
        ...
        if parallel_config.all2all_backend == "nixl_ep":
            ...
            # Initialize PeerActiveStateManager (idempotent if some other
            # path on the Worker initialized it earlier).
            ...
            # Initialize DPFTGlooManager. Creates its own TCPStore via
            # `stateless_init_dp_group(return_store=True)`. The 4 Workers
            # rendezvous on the master port via the coord-store
            # mechanism, exactly like the Actor side does. The resulting
            # store is local to the Worker-side FT-gloo group and is
            # independent of the Actor-side one (each side maintains
            # its own sub-group + state).
