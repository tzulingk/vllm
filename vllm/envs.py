...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # NIXL EP dispatch/combine RDMA receive timeout (ms). Governs how long the
    # GPU all-to-all kernel waits for bytes from each source rank before flipping
    # that rank's mask bit. Wired into the nixl_ep Buffer ctor; the library's own
    # default is 30000ms, which stalls the survivor's forward for ~30s on a dead
    # peer (DYN-3441). Kept at the FT design's assumed 5s window (see dp_utils).
