...
#### Bug found in Shape 2 v1: leader-cleanup race wedged slow joiners

The first Shape 2 implementation (commit `b8df573ef`, plus key/unblock fixes `a874d37b6`, `d7037e1dc`) had a subtle race in `_staged_barrier`. After every survivor passed the TCPStore arrival-key poll, the leader `delete_key`'d each survivor's `arrival_*_<r>`. If a slow joiner (the last one to announce) was scheduled away between announcing arrival and entering its own poll loop, by the time it checked for the *other* survivors' arrival keys those had already been deleted by the leader. The slow joiner's poll never converged, hit the 5s first-attempt timeout, then on retry used `timeout=None` and deadlocked forever in the polling loop.

Observed in the 2026-05-28 kill-DP1 run: DP3 announced 200µs after DP0/DP2, then went silent for the rest of the run (no further log lines from the DP3 actor). DP0/DP2 finished REDISTRIBUTE in 500ms and resumed dispatching; DP3 was wedged in Python so its expert kernel never launched; NIXL EP writes to DP3's buffer never got ack'd → kernel false-flagged DP3 → cascade.

Why elastic-EP doesn't have this bug: its `_staged_barrier` follows the TCPStore poll with `torch.distributed.barrier(dp_group)`. That collective enforces all-ranks-here before the leader cleanup, closing the race. We had removed that line specifically because `dp_group` still includes the dead rank and the collective would hang.

#### Shape 2 v2: FT-gloo all_reduce as the survivors-only barrier (commit `0e8bf082d`)

Replace the TCPStore poll + leader cleanup with a single `all_reduce` on the FT-gloo survivors-only sub-group:

```python
ft = DPFTGlooManager.instance()
survivor_mask = [1 if r != self.dead_dp_rank else 0
                 for r in range(self.dp_world_size)]
dummy = torch.zeros(1, dtype=torch.float32)
_, valid = ft.all_reduce(dummy, ReduceOp.SUM, active_mask=survivor_mask,
                          timeout_ms=timeout_ms)
```

`FaultTolerantGlooGroup.all_reduce` rebuilds the underlying gloo sub-group to match `active_mask` if it differs from the previous call. On the first `FtDyingPeerState` event this rebuild happens here, producing exactly the survivors-only group that EPLB cross-DP collectives need in REDISTRIBUTE. Side benefit: no extra explicit rebuild later.

This mirrors elastic-EP's pattern (TCPStore poll → real collective barrier → leader cleanup) but uses a sub-group that excludes the dead rank, so `torch.distributed`-level semantics work without hanging.

Empirically verified 2026-05-28: kill-DP1 with this commit → all 3 survivors complete `ENTER_BARRIER → REDISTRIBUTE → COMPLETE` in ~650ms. FT-gloo gen=2 active=[0,2,3] is installed during REDISTRIBUTE. DP3 emits normal logs throughout (no wedging).

The cascade still fires at +25s after kill, but for a *different* reason (kernel auto-flips innocent ranks when a peer dies; L3 OR-consensus accepts a single-engine report). That's a separate problem, tracked under "L3 — silent-failure escalation" below.
...
### 2026-05-28 — Shape 2 v2 (FT-gloo barrier) result: leader-race closed, kernel-cascade still present

Commit `0e8bf082d`. Image `sha256:91ebcfa...` (VLLM_USE_PRECOMPILED=1, ~13min total wall).

Kill DP1 with 5 pre-kill + 5 post-kill concurrent curls. Pre-kill 5×200 @0.57s. Post-kill 4×200 @34-35s + 1×60s timeout.

**State machine (all wall-times Δ from kill at 1780025298.785):**

| Δt | engine | event |
|---|---|---|
| +71ms | all survivors | notify_engine_death(1) received, FtDyingPeerState created, announced arrival |
| **+113ms** | **DP3** | **barrier passed → REDISTRIBUTE (t_since_create=0.043s) ← was missing in run 1** |
| +113ms | DP2 | barrier passed → REDISTRIBUTE |
| +113ms | DP0 | FT-gloo rebuild gen=2 active=[0,2,3] |
| +570ms | DP2 | COMPLETE (redistribute_took=0.457s) |
| +650ms | DP0 / DP3 | COMPLETE |

All three survivors completed the state machine in ~650ms. FT-gloo group rebuilt to active=[0,2,3] during REDISTRIBUTE. The leader-cleanup race that wedged DP3 in run 1 is closed.

**Kernel cascade still fires (separate bug):**

| Δt | event |
|---|---|
| +9.4s | DP0 L3: rank 3 flagged (false), cleared bit, probing |
| +15s | DP2 L3: rank 3 flagged (false) |
| +20s | DP2 L3: rank 0 flagged (false) |
| +25s | OR-consensus broadcasts notify_engine_death(3) — **false cascade** |
| +30s | DP0/DP2 enter FtDyingPeerState for the (alive) DP3 |
| +35s | OR-consensus also flags DP2 dead |
| +59s | FT-gloo rebuild gen=3 active=[**0**] — cluster collapsed to single rank |

Two distinct bugs identified, one fixed in this commit:

1. ✅ **Solved by `0e8bf082d`:** DP3 wedged at the leader-cleanup race in the TCPStore barrier.
2. ❌ **Still present:** kernel auto-flips innocent neighbors on a real-peer death (warp-to-rank timeout attribution), AND L3 OR-consensus accepts a single engine's report as enough to broadcast a death.

Next experiments in priority order:

1. **`VLLM_FT_EP_CONSENSUS=AND` (no rebuild).** Already implemented in `_consensus_dead_set` — env-var-only test. AND requires every live engine to report a rank as degraded; DP3 itself never flags DP3 (kernel only flags peers), so AND-consensus on DP3 = ∅. Cheapest test.
2. **L4 — proactive mask sync** at the end of REDISTRIBUTE: explicit `update_mask_buffer(N, mask=False)` for every alive rank. Closes the window where the kernel sees DP3 as "unresponsive" while DP3 has no batch.
3. **Asymmetric-work investigation.** Determine whether DP3 needs a "receive-only" expert kernel even when it has no inference batch, to drain NIXL EP buffer slots written by DP0/DP2.

### 2026-05-29 — Actual root cause found via py-spy: Worker-side FT-gloo singletons not initialized → raw `dist.all_reduce` on dead-rank-inclusive `dp_group` hangs

Runs 3-5 with progressively richer instrumentation (warmup loop, RPC enter/exit logs, dummy-batch START/DONE logs, exhaustive `run_busy_loop` shutdown-trace logs) eliminated several earlier hypotheses (JIT-mid-kill, `input_queue.get(block=True)` race) and pinpointed the actual hang location.

In run-5 DP3's actor was still **inside `actor.run()`** (process tag `ray::DPMoEEngineCoreActor.run`, not back in Ray main_loop) and DP3's worker was **alive**. py-spy on the worker showed:
...
Thread 9258 (idle): "MainThread"
    ...
    ft_or_raw_all_reduce (vllm/distributed/elastic_ep/ft_gloo.py:341)   ← raw fallback path
...
`ft_or_raw_all_reduce` at line 341 is the **fallback** branch:
...
`DPFTGlooManager.init(...)` is called **only** inside `DPEngineCoreProc.__init__` (the **Actor** process). The **Worker** is a separate Ray-spawned subprocess and its module-level singletons (`DPFTGlooManager._instance`, `PeerActiveStateManager._instance`) stay `None`. So the Worker falls through to the raw `dist.all_reduce(tensor, group=dp_group)` on the original DP-world-size gloo group — which still contains the dead peer — with no timeout.

#### Why DP3 hangs but DP0/DP2 raise (same code, different runtime fate)

Both Workers run the same fallback line. The divergence is a gloo TCP socket-state race:

- DP0/DP2 Workers were continuously doing dp_group I/O up to the kill. By the time the post-kill `all_reduce` ran, gloo had already observed the dead peer's FIN. The next send fails fast → torch wraps it as `ValueError("Process group is not initialized in the world group map")` → caught at [`dp_utils.py:79`](vllm/v1/worker/dp_utils.py#L79) → degraded fallback ("FT EP: DP_run_ar all_reduce failed ...").
- DP3 Worker was idle during the FT recovery window (REDISTRIBUTE is local CPU + a worker-internal RPC; no dp_group I/O). When DP3's post-recovery `all_reduce` finally fired, the gloo socket to DP1 was in a half-closed state — the send went into the OS buffer, the recv blocked, no vllm-level timeout → DP3 hangs indefinitely.

#### Fix A (commit `11bd286e1`) — initialize FT-gloo singletons on the Worker

`Worker.__init__` now mirrors the Actor-side pattern when `all2all_backend == "nixl_ep"`:
...
After this, the Worker takes the FT-aware path: `ft.all_reduce(tensor, op, active_mask=survivors_only)` which rebuilds a survivors-only sub-group excluding the dead rank. The hang is structurally impossible because the dead peer is not in the active set.

The previous "VLLM_FT_EP_CONSENSUS=AND" / L4 / asymmetric-work items above remain candidate refinements if any cascade behavior is still observed after Fix A — but Fix A addresses the actual hang.

---

## Minimal kernel-mask repro branch (`ft-nixl-ep-kernel-mask-repro`)

A separate, much smaller branch was created to isolate the NIXL EP kernel-mask divergence question on its own. Branched off upstream `4e597b749` + only the kernel-mask consensus check — no FT cascade-fix code, no L3, no FtDyingPeerState, no apply_kernel_mask.

**HEAD:** `8a6e99003` (4 commits on top of upstream).
**Image:** `nvcr.io/nvidian/dynamo-dev/tzulingk-vllm:kernel-mask-repro` @ `sha256:e9ebff7217a6...`.
**Diff:** <https://github.com/tzulingk/vllm/compare/4e597b749144d4b3f0716994b95486ab734b185f...ft-nixl-ep-kernel-mask-repro>

### Three additions

1. `NixlEPAll2AllManager.query_mask()` — wraps `buffer.query_mask_buffer(buf)` into a reusable CPU tensor.
2. `Worker.query_nixl_ep_mask()` — `collective_rpc` hook for the engine-core.
3. `DPEngineCoreProc._verify_kernel_mask_consensus_or_crash()` — called from `_has_global_unfinished_reqs` on every call. Publishes the local mask under `nixl_kernel_mask_dp{rank}_wave{N}` on the dp_store, reads peers' wave-N keys, crashes on disagreement.

### Key design points

- **Wave-tagged keys**: each rank's wave-N mask is published under its own key so wave-N never gets overwritten when a rank advances to wave-N+1.
- **`VLLM_FT_EP_REPRO_DEAD_DP_RANKS=1`** env var: excludes the to-be-killed rank so the check doesn't burn the full 3 s `store.wait` waiting for a key that will never be written.
- **Truncation to `dp_world_size * tp_size` slots**: the NIXL EP buffer is sized to `buffer.group_size=32` (elastic-EP) but only the first `num_ep_ranks` slots correspond to actual EP ranks. Tail slots are unused and carry inconsistent sentinels (DP0 reports `-1`, DP2/DP3 report `0`) — see "NIXL EP unused-slot inconsistency" below. We slice to the meaningful prefix before publish + compare.
- **`logger.info` dump on every successful match** so the operator can see the per-rank masks every check, not only on crash.

### NIXL EP unused-slot inconsistency (probable NIXL bug)

First run on this branch crashed at wave=0:

- `dp0: mask=[0, 0, 0, 0, -1, -1, ..., -1]`
- `dp2: mask=[0, 0, 0, 0,  0,  0, ...,  0]`
- `dp3: mask=[0, 0, 0, 0,  0,  0, ...,  0]`

All three agree on slots 0–3 (`[0,0,0,0]` = all alive). They disagree only on **unused** slots 4–31. NIXL EP's documented `query_mask_buffer` contract is "1=dead, 0=alive"; there is no documented sentinel for unused-slot bytes, so the `-1` from DP0 is undefined behavior that diverges from DP2/DP3.

Worth a NIXL ticket — either standardize the unused-slot value or document it as undefined.

### Test outcome (REPRO=1, image `e9ebff72`)

- vllm serves correctly: 10 verification curls returned "Paris" in 71–655 ms.
- **771+ consensus checks pre-kill, 0 divergence.** Every check: all DP ranks reported the same `[0, 0, 0, 0]` mask.
- Killed `Worker_DP1_EP1`. 5 post-kill curls timed out at 60 s (expected — this branch has no FT-gloo wrapper, so cross-DP `all_reduce` on the dead peer hangs).
- After kill, the check logged "wave-6 MISSING for ranks [2,3]" — DP2/DP3 stuck in `_run_ar`, never reached wave-6. Returned without crashing.
- No `DPMoEEngineCoreActor` died.

**Verdict for this config (DP=4, TP=1, DeepSeek-V2-Lite, normal traffic):** the NIXL EP kernel masks do not diverge in steady-state operation. Post-kill, we couldn't probe further because the cluster hangs on `_run_ar` before any new dispatch/combine kernel fires.

To probe cascade behavior post-kill, the natural next step is either:

1. Port the Fix-A FT-gloo wrapper to this branch (survivor-stays-alive), or
2. Build a standalone NIXL EP harness (no vllm) to hand to the NIXL team.

### Update: cascade-guard + gloo-timeout finding (kernel-mask-repro branch, commit `2dffe77bf`)

A minimal cascade guard in [`vllm/v1/worker/dp_utils.py::_run_ar`](vllm/v1/worker/dp_utils.py) (a `try/except (RuntimeError, ValueError)` around `dist.all_reduce`) is enough to keep DP0/DP2/DP3 survivors alive after killing DP1. The full kill test produced:

| Rank | Wave reached | Last observed kernel mask | When relative to kill |
|---|---|---|---|
| DP0 | 2 | `[0, 1, 0, 1]` (DP1 + DP3 dead per kernel) | ~immediately |
| DP2 | 2 | `[0, 1, 0, 1]` (same as DP0) | ~immediately |
| DP3 | 2 | `[0, 0, 0, 0]` (kernel never observed kill) | **28 minutes later** |

Our wave-tagged consensus check **did detect the real cross-rank divergence**, but only after DP3 caught up.

#### Why DP0/DP2 raise but DP3 hangs

`dist.all_reduce` in `_run_ar` runs over the **gloo** backend (`disable_nccl_for_dp_synchronization=True` takes the CPU branch). For our small 64-byte payload, gloo uses ring-allreduce: `DP0 -> DP1 -> DP2 -> DP3 -> DP0`. Each rank only communicates with its two ring neighbors — so DP3 never directly talks to DP1 in the data plane.

At kill time:

- DP0 was `send`ing to DP1 (its successor): kernel returned `EPIPE` on the FIN'd socket → gloo raised → cascade guard caught.
- DP2 was `recv`ing from DP1 (its predecessor): kernel returned `0` (EOF) → gloo raised → cascade guard caught.
- DP3 was `recv`ing from DP2 (its predecessor): **DP2's socket to DP3 is alive** (DP2's process didn't die), but DP2 had already abandoned its allreduce after raising. DP2 never sends the next ring chunk to DP3. DP3's recv blocks forever — there's no FIN to surface, just silence. The cascade guard's `try/except` is useless because the call never raises, it hangs.

This is the **failure-stop vs. dead-quiet** distinction: gloo + cascade guard handle direct failures but not silent neighbor-abandonment.

#### The 28-minute lag = gloo's default ProcessGroup timeout

Gloo's default timeout for a `ProcessGroup` is 1800 s (30 minutes). At ~28 minutes post-kill that timer fired, gloo's underlying recv raised, the cascade guard caught it, DP3 advanced to wave 2, and the consensus check detected the divergence.

#### Fix: `--cpu-distributed-timeout-seconds`

Upstream's `ParallelConfig` has a `cpu_distributed_timeout_seconds` field (default `None`, meaning "use gloo's 1800s"). It's read in [`vllm/distributed/utils.py::get_cpu_distributed_timeout_or_none()`](vllm/distributed/utils.py#L494) and passed to `init_gloo_process_group(timeout=...)` ([`vllm/distributed/utils.py:611-617`](vllm/distributed/utils.py#L611-L617)).

CLI:

```bash
vllm serve ... --cpu-distributed-timeout-seconds 10
```

With this, DP3-style hangs fail fast (10 s) → cascade guard catches → DP3 advances to wave 2 within seconds → the consensus check detects divergence in ~10 s instead of 28 min.

**Note**: the env var `VLLM_CPU_DISTRIBUTED_TIMEOUT_SECONDS` exists only in our `ft-nixl-ep-demo` branch's FT-gloo wrapper code — upstream silently ignores it. **Use the CLI flag** in any kill-recovery test on the upstream or minimal-repro branches.

### Result with `--cpu-distributed-timeout-seconds 10` (Run 11)

Repeating the kill-DP1 test with the CLI flag added:

| | Run 10 (no flag) | Run 11 (flag=10) |
|---|---|---|
...
| Diverging rank | DP3 (idle) | DP0 (idle this run) |
| False-flag of innocent peer | YES (DP3 falsely dead per DP0/DP2 kernel after 28 min cascade) | **NO** — kernel correctly flags only DP1 |

Run 11 crash dump:

```
NIXL EP KERNEL MASK REPRO -- divergence detected at wave=2 on dp_rank=2:
    dp0: mask=[0, 0, 0, 0] ts=+0.100s wave=2
    dp2: mask=[0, 1, 0, 0] ts=+0.000s wave=2
    dp3: mask=[0, 1, 0, 0] ts=-0.000s wave=2
```

The "rank that doesn't see DP1 dead" is **whichever rank was idle at kill time**. Kernel-mask divergence is real but, with a short gloo timeout, it's a passive observation gap (idle rank never ran dispatch with DP1 → didn't observe the timeout), not a kernel cascade. The cascade-style false-flagging seen in Run 10 (DP3 falsely flagged dead) was an **artifact of the 28-minute stalled state**, not a fundamental kernel bug.

### Run 12 + root cause: CPU-tensor race in `query_mask_buffer`

Run 12 reused the Run 11 setup but added two pieces of instrumentation:

1. A load-balancer routing log to confirm post-kill traffic distribution per DP rank.
2. A grep over the kernel's own `printf` warnings inside the dispatch timeout branch ([`nixl_ep_ll.cu:320-328`](https://github.com/ai-dynamo/nixl/blob/5c801155f7b1561992e35aae084bcefff717cd34/examples/device/ep/csrc/kernels/nixl_ep_ll.cu#L320-L328)).

The printfs showed **24 distinct `NIXL-EP timeout for dispatch receive, rank 0, …, src_rank=1`** lines from DP0's worker in the 44-second post-kill window. Each printf sits in the same `if` block as `atomicExch(mask_buffer_ptr + src_rank, 1)`, so DP0's kernel executed the mask-flip 24 times. Yet `query_mask_buffer` on the same rank read `mask[1] = 0`. This was filed as [DYN-3139](https://linear.app/nvidia/issue/DYN-3139/nixl-ep-kernel-mask-atomicexch-writes-from-dispatch-timeout-do-not).

The NIXL team root-caused it in [itayalroy/vllm@4d06e2c9](https://github.com/itayalroy/vllm/commit/4d06e2c9b6e9e62377d25e0818801f14436bf99a): we were passing a **pageable CPU tensor** as the destination to `query_mask_buffer`. The fix is to allocate the destination on the current CUDA device and use `.cpu()` for the synchronized copy back:
...
#### Why a CPU destination tensor races the read

Three concepts up front:

1. **GPU memory and CPU memory live in different physical places**, connected only through the PCIe bus. CUDA kernels read/write GPU memory by default; the CPU reads CPU memory. Moving bytes between them requires a physical transfer across PCIe.
2. **CPU memory comes in two flavors.**
   - *Pageable* (default): the OS may move it around freely.
   - *Pinned / page-locked*: a fixed physical address the GPU can DMA into directly.
   PyTorch's default `torch.zeros(..., device="cpu")` returns **pageable** memory. Kernels can address pageable memory via UVA, but writes go through write-combining buffers and PCIe transactions with no guaranteed ordering relative to the host CPU's read.
3. **CUDA kernels run asynchronously on a stream.** A kernel launch enqueues work; it doesn't wait for completion. To force the CPU to wait, you need an explicit sync.

What happened, step by step, in the buggy version:

1. Python allocated `buf` in pageable CPU memory. Call its address `addr_cpu`.
2. `query_mask_buffer(buf)` launched a CUDA kernel on the current stream. The kernel was told: store `mask_buffer_ptr[i]` into `addr_cpu + i*4`.
3. Python returned from `query_mask_buffer` **immediately** — the kernel was only enqueued.
4. Python ran `buf.clone()`, which does a plain CPU-side read of `addr_cpu`.
5. **Race**: at the moment of step 4, the kernel from step 2 may not have run yet, may be partway through, or may have completed but with its writes still in flight on PCIe / in write-combining buffers, not yet visible to the CPU. There was no synchronization. Sometimes the CPU read landed after the writes arrived (we saw the correct mask). Sometimes it landed before (we saw the pre-launch `zeros(...)` contents → `[0, 0, 0, 0]`).

The fix moves the destination onto GPU memory (a normal GPU-to-GPU write, well-defined and fast), then uses PyTorch's `.cpu()` which performs a `cudaMemcpyAsync` followed by an explicit stream synchronization. By the time `.cpu()` returns, the kernel has finished, the bytes are on the host, and the CPU can safely read them.

The bug is **not** in the NIXL kernel — the atomicExch writes were always persisting correctly on the GPU. The bug was in how vLLM was *reading* the result: a missing synchronization between the GPU's writes and the CPU's read.

#### Implications

- **DYN-3139's dispatch-driven-erasure theory was a phantom.** Run 10 (mask correctly flipped) and Run 12 (mask read as zero) weren't different kernel behaviors — they were different points along the unsynchronized-read race spectrum. Run 10's 28-minute stall gave the unsynchronized write plenty of time to land before the eventual read; Run 12's tight timing did not.
- **The kernel-mask consensus design can work after this fix.** With the read synchronized, all surviving ranks should see consistent mask state for the dead peer. Validating that empirically is the next step.
- **Lesson generalizes**: any time you pass a host tensor to a NIXL/CUDA API that fills it via a kernel, you need either a pinned-memory destination plus a stream sync, or (simpler) a GPU destination plus `.cpu()` afterward.
